...
void AWebcamRenderer::BeginPlay()
{
	...
	std::string ip = std::string(TCHAR_TO_UTF8(*IPAddress));
	boost::asio::ip::tcp::endpoint serverEndpoint(boost::asio::ip::address::from_string(std::string(ip)), 31401);
	...
	if (isStreamOpen && frame.data)
	{
		...
		for (int y = 0; y < VideoSize.Y; y++)
		{
			for (int x = 0; x < VideoSize.X; x++)
			{
				...
				Data[i].B = frame.data[i * 4 + 0];
				Data[i].G = frame.data[i * 4 + 1];
				Data[i].R = frame.data[i * 4 + 2];
